...
/**
 * @brief Hàm callback ngắt timer - PHIÊN BẢN CUỐI
 */
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  if (htim->Instance == TIM2)
  {
    ...
    // -------- Quét LED 7-đoạn mỗi 250ms (nhanh hơn để hiển thị mượt) --------
    if(isTimerExpired(0)){
      setTimer(0, 25);    // 25 × 10ms = 250ms - quét nhanh để tránh nhấp nháy
      scanLED();          // Quét qua tất cả LED 7-đoạn
    }

    // -------- Nhấp nháy LED chấm mỗi 500ms (như dấu hai chấm trong đồng hồ) --------
    if (isTimerExpired(1))
    {
      setTimer(1, 50);    // 50 × 10ms = 500ms
...
/**
 * @brief Chương trình chính - ĐỒNG HỒ DIGITAL
 */
int main(void)
{
  /* Cấu hình MCU */
  ...
  setTimer(1, 75);        // Timer 1: 50 × 10ms = 500ms cho LED chấm
  ...
  while (1) {
    ...
    updateClockBuffer();  // Cập nhật dữ liệu hiển thị sau mỗi giây
  }
}
/**
